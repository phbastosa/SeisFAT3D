...
for i in range(len(vp)):
    ...
    M[0,0] = pax*pax; M[0,1] = pay*pay; M[0,2] = paz*paz  
    M[1,0] = pbx*pbx; M[1,1] = pby*pby; M[1,2] = pbz*pbz  
    M[2,0] = pcx*pcx; M[2,1] = pcy*pcy; M[2,2] = pcz*pcz
    
    M[3,0] = pbx*pcx; M[3,1] = pby*pcy; M[3,2] = pbz*pcz 
    M[4,0] = pcx*pax; M[4,1] = pcy*pay; M[4,2] = pcz*paz 
    M[5,0] = pax*pbx; M[5,1] = pay*pby; M[5,2] = paz*pbz
      
    M[0,3] = 2.0*pay*paz; M[0,4] = 2.0*paz*pax; M[0,5] = 2.0*pax*pay  
    M[1,3] = 2.0*pby*pbz; M[1,4] = 2.0*pbz*pbx; M[1,5] = 2.0*pbx*pby
    M[2,3] = 2.0*pcy*pcz; M[2,4] = 2.0*pcz*pcx; M[2,5] = 2.0*pcx*pcy

    M[3,3] = pby*pcz + pbz*pcy; M[3,4] = pbx*pcz + pbz*pcx; M[3,5] = pby*pcx + pbx*pcy
    M[4,3] = pay*pcz + paz*pcy; M[4,4] = paz*pcx + pax*pcz; M[4,5] = pax*pcy + pay*pcx
    M[5,3] = pay*pbz + paz*pby; M[5,4] = paz*pbx + pax*pbz; M[5,5] = pax*pby + pay*pbx
